...
// 1. Map isUpcoming
const oldMapRegex = /key: g\.key,\s*label: g\.label,\s*isLocked: !isGroupEnabled\(g\.key\) \|\| !platformFeatures\.includes\(g\.key\),/;
const newMap = `key: g.key,
        label: g.label,
...
const oldLabelRegex = /<span className="flex-1 font-medium text-slate-300 group-hover\/groupbtn:text-white tracking-wide text-sm ml-2 flex items-center justify-between pr-2">\s*\{t\(g\.label\)\}\s*\{\(g as any\)\.isLocked && <Lock className="h-3\.5 w-3\.5 text-amber-500" title="Upgrade to use this feature" \/>\}\s*<\/span>/m;
...
code = code.replace(oldLabelRegex, newLabelDiv);

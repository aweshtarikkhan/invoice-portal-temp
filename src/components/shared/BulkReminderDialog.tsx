...
export function BulkReminderDialog({
  ...
  const handleSend = async (row: any) => {
    ...
    if (!client?.email) {
      toast({ title: "No email", description: `${client?.display_name || "Client"} has no email on file.`, variant: "destructive" });
      return;
    }

    try {
      await supabase.functions.invoke("send-document-email", {
        body: { entityId: row.id, entityType: "invoice", recipientEmail: client.email }
      });
      toast({ title: "Email Sent!", description: "Reminder email dispatched successfully." });
    } catch (e: any) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Send Email reminders one-by-one. Each click sends an email with the secure portal link.
          </DialogDescription>
        </DialogHeader>
        {loading ? (

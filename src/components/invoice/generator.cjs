...
const baseContent = (name, primaryColor, accentColor, headerStyle) => `import React, { useMemo } from "react";
  ...
  return (
    <div className="w-full h-full bg-white text-black p-8 font-sans" style={{ fontSize: "11px", color: "#333" }}>
      ...
      <div className="flex justify-between items-start mb-4">
        <h1 className="text-xl font-bold uppercase tracking-wider mb-2">{getTitleText()}</h1>
      </div>
      <div className="grid grid-cols-2 gap-4 border border-gray-400 mb-4">
        ...
        <div className="grid grid-cols-3 gap-2 p-2 text-[10px]">
           ...
           {poNumber && (
             <div>
               <p className="font-semibold text-gray-500">P.O. No.</p>
           ...
           {customFields.slice(0, 3).map((cf: any, idx: number) => (
             <div key={idx}>
               <p className="font-semibold text-gray-500">{cf.name}</p>
...
fs.writeFileSync(path.join('src/components/invoice', 'ClassicTabularInvoiceTemplate.tsx'), baseContent('ClassicTabularInvoiceTemplate', '#1e293b', '#334155', 'tabular'));
fs.writeFileSync(path.join('src/components/invoice', 'ModernNavyInvoiceTemplate.tsx'), baseContent('ModernNavyInvoiceTemplate', '#1e3a8a', '#eab308', 'modern'));
fs.writeFileSync(path.join('src/components/invoice', 'ModernTealInvoiceTemplate.tsx'), baseContent('ModernTealInvoiceTemplate', '#0f766e', '#14b8a6', 'modern'));
fs.writeFileSync(path.join('src/components/invoice', 'ModernCrimsonInvoiceTemplate.tsx'), baseContent('ModernCrimsonInvoiceTemplate', '#be123c', '#f43f5e', 'modern'));

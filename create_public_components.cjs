...
const srcDir = 'src';
const componentsDir = path.join(srcDir, 'components', 'public');
if (!fs.existsSync(componentsDir)) {
  fs.mkdirSync(componentsDir, { recursive: true });
}

// 1. Create PublicHeader.tsx
const headerCode = `import { Link } from "react-router-dom";
...
`;
fs.writeFileSync(path.join(componentsDir, 'PublicHeader.tsx'), headerCode);
...
`;
fs.writeFileSync(path.join(componentsDir, 'PublicFooter.tsx'), footerCode);
console.log('Created PublicHeader.tsx and PublicFooter.tsx');
